                    ...
                    <div onclick="window.location='{{ url('/article/' . $i->id) }}'" class="relative bg-[#DED2F9] rounded-[1.71rem] py-[1.71rem] px-[2.14rem] flex flex-col gap-[3.42rem] mx-4">
                        ...
                        <div class="flex flex-col justify-between h-full gap-[1.23rem]">
                            <div class="flex flex-col gap-[5px] max-w-[36.21rem]">
                                ...
                                <p class="text-[#656F77] raleway text-[1.2rem]" style="display: -webkit-box;
                                                                      -webkit-line-clamp: 3;
                            ...
                            @auth
                                @if(auth()->user() && auth()->user()->role === 'Администратор')
                                    <div class="w-full flex items-center justify-between">
                                        <a href="{{ route('articles.edit', $i->id) }}"
                                        ...
                                        <form action="{{ route('articles.destroy', $i->id) }}" method="POST" style="display:inline;">
                                            @csrf @method('DELETE')
                                ...
                                @else
                                    <button onclick="window.location='{{ url('/article/' . $i->id) }}'" class="flex items-center gap-3 raleway-semibold text-[1.2rem]">
                                        Читать далее
