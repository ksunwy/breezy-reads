<x-layout>
    <section
        ...
        <form method="POST" action="{{ route('register') }}" class="flex flex-col gap-4 py-5 px-6">
            @csrf
            <h1 class="poppins-bold text-[2.73rem]">Присоединиться</h1>
            ...
            <div class="relative w-full">
                <input placeholder="Повторить пароль" type="password" name="password_confirmation"
                    class="bg-white border border-solid border-[#ACB8C2] p-3 rounded-sm min-h-10 text-[1.2rem] poppins w-full">
                <button class="toggle-password absolute top-1/2 -translate-y-1/2 right-3">
                    <svg class="eye-icon" width="16" height="17" viewBox="0 0 16 17" fill="none"
                        xmlns="http://www.w3.org/2000/svg">
            ...
            <span class="w-full flex items-center justify-center text-[1.2rem] gap-2">
                <span class="text-[#656F77]">Уже есть аккаунт?</span>
                <a href="/login" class="poppins-bold transition-all duration-300 hover:text-[#656F77]">Войти</a>
            </span>
        </form>
        @if ($errors->any())
            <div style="color:red">
