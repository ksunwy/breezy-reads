<form method="GET" action="{{ route('books.index') }}" class="w-full bg-white shadow lg:max-w-[calc(100%-17.97rem)] -mt-10 rounded-xl p-8 grid grid-cols-2 md:grid-cols-3 gap-6 items-end mb-12">
    ...
    <div class="flex gap-3 mt-auto">
        <button type="submit" class="bg-[#191D21] text-white rounded-lg px-6 py-3 text-[1.3rem] hover:bg-gray-800 transition w-full">
            Применить
        </button>
        <a href="{{ route('books.index') }}" class="bg-gray-200 text-gray-800 rounded-lg px-6 py-3 text-[1.3rem] hover:bg-gray-300 transition w-full text-center">
            Сбросить
        </a>
    </div>
