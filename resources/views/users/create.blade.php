<x-layout>
    <section
        class="absolute top-1/2 -translate-x-1/2 left-1/2 -translate-y-1/2 p-10 bg-[#FFDBF5] h-fit w-fit lg:px-[3.68rem] lg:pt-[4.88rem] lg:pb-8 flex flex-col gap-[1.37rem] lg:gap-[2.91rem] max-h-[100dvh] rounded-[1.71rem]">
        ...
        <form method="POST" action="{{ route('users.store') }}" class="flex flex-col gap-4 py-5 px-6">
            @csrf
            <h1 class="poppins-bold text-[2.73rem] leading-[110%]">Создать пользователя</h1>
